...
setwd("/home/ziyang/Documents/CMEECourseWork/Ecological_Evolution/results") 
...
# for the first site
group8_na <- group8_all_predictions_df %>% drop_na()
g8_subset <- subset(group8_na, group8_na$confidence>0.7) 
...
group8_diversity <- audio_restricted %>% 
  summarize(Species_Richness = n_distinct(common_name), #Finds the number of unique names 
